...
userSchema.pre('findOne', async function() {
  try{
    ...
    console.log('there is an error in pre find async func');
  }
});
...
userSchema.pre('save', async function () {
  ...
  try {
    let userRole = await userSchema.findOne({ role: this.role });
    if (!userRole) {
      userRole = new Role({ role: this.role, capabilities: userSchema.capabilities});
      await userRole.save();
    }
